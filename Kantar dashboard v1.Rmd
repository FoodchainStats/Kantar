...
cleaned_data_initial <- read.csv("S:/Analysis/Covid-19/Kantar/dashboard/Current_Week_cleaned_data.csv", stringsAsFactors = F)
...
# allows user to select demographic of interest (not including social class by region)
selectInput(
  "demog", label = HTML('<b>Demographic Group:</b>'),
 # choices = c("Lifestage", "Region", "Social Class", "Vulnerable Groups"), selected = "Lifestage"
 choices = as.character(unique(cleaned_data$Group[cleaned_data$Group != "Social Class by Region"])), selected = "Social Class"
)
...
df <-  filter(selected_cleaned_data(), Category == category() & Indicator == indicator_demog() & Group == demog())%>%
 # filter(selected_cleaned_data(), Category == "Total Food & Drink" & Indicator == "Volume" & Group == "Social Class")%>%
   pivot_longer(cols = starts_with("w.e"),
                 names_to = "Week",
                 values_to = "Value")%>%
    # turning week names into dates
...
names(cols) <-  selected_demog_data()%>%
  ...
  p <- ggplot(selected_demog_data(), aes(x = Week, y = Value, group = Sub_Group, colour = Sub_Group, 
    ...
    labs(x = "Week", y = paste0(indicator_demog(), " (", unit_demog(), ")"),
         title = paste0(indicator_demog(), " - ", demog(), " - ", ifelse(category() == "Total Food & Drink", "All food & drink categories", category()))) +
    # adding extra info to chart
...
renderPlotly({
  ...
  layout(title = list(text = paste0('<b>', input$pct_comparator_demog, '</b>',
                                    '<br>',
                                    '<sup>',
                                    indicator_demog(), " - ", demog(), " - ", ifelse(category() == "Total Food & Drink", "All food & drink categories", category()),
                                    '</sup>')))
})
...
demog_summary <- reactive({
 filter(group_summary(), Category == category() & Group == demog() & Indicator == indicator_demog())%>%
...
- The analysis is using commercially available data from Kantar Worldpanel’s take home consumer panel. The data is provided on a weekly basis dating back to January 2019. 
